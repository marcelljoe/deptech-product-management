...
const ProductsCategory = (props: any, { fallback }: any) => {
  ...
  let url = `http://localhost:5007/api/product_category/list?row=${states.data.dataPerPage}&page=${states.data.currentPage}&key=${states.filter.key}&column=${states.filter.columns}&direction=${states.filter.directions}`
    ...
    <>
      <SWRConfig value={{ fallback }}>
        ...
        <Card
          ...
          title="List of Products Category"
          extra={
        ...
        <Modals
          ...
          header={states.typeModal == "Add" ? "Add Category" : "Update Category"}
...
const saveThis = async (data: any) => {
  let res: any = await axios.post(`http://localhost:5007/api/product_category/saveCategory`, {
    name: data.name,
...
const updateThis = async (data: any) => {
  let res: any = await axios.put(`http://localhost:5007/api/product_category/edit`, {
    name: data.name,
    description: data.description,
    id: data.id
  });
...
const deleteThis = async (data: any) => {
  let res: any = await axios.delete(`http://localhost:5007/api/product_category/deleted/${data.id}`);
  return res
};
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  const getList: any = await axios.get(`http://localhost:5007/api/product_category/list?key=${params.key}&page=${params.page}&row=${params.row}&column=${params.column}&direction=${params.direction}`, {
    headers: {
  ...
  return {
    props: {
      fallback: {
        "/api/product_category/list": JSON.parse(JSON.stringify(data)),
      },
